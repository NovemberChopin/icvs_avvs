...
public:
  ...
  bool GetModelConfig(const std::string& top_model_name,
                      ModelConfig** model_config);
  bool SetModelConfig(const std::string& model_name,
                      ModelConfig** model_config);
...
public:
  ...
  bool Reset(const avos::common::config::ModelConfigProto& proto);
  bool SaveToProto(avos::common::config::ModelConfigProto& proto);
  ...
  void AddValue(const std::string& name, int value) {
    integer_param_map_[name] = value;
  }

  void AddValue(const std::string& name, std::string value) {
    string_param_map_[name] = value;
  }

  void AddValue(const std::string& name, double value) {
    double_param_map_[name] = value;
  }

  void AddValue(const std::string& name, float value) {
    float_param_map_[name] = value;
  }

  void AddValue(const std::string& name, bool value) {
    bool_param_map_[name] = value;
  }
  ...
  bool ChangeValue(const std::string& name, std::vector<int> values) {
    return ChangeValueFromMap<std::vector<int>>(name, array_integer_param_map_,
           values);
  }

  bool ChangeValue(const std::string& name, std::vector<double> values) {
    return ChangeValueFromMap<std::vector<double>>(name, array_double_param_map_,
           values);
  }

  bool ChangeValue(const std::string& name, std::vector<float> values) {
    return ChangeValueFromMap<std::vector<float>>(name, array_float_param_map_,
           values);
  }

  bool ChangeValue(const std::string& name,
                   std::vector<std::string> values) {
    return ChangeValueFromMap<std::vector<std::string>>(
             name, array_string_param_map_, values);
  }

  bool ChangeValue(const std::string& name, std::vector<bool> values) {
    return ChangeValueFromMap<std::vector<bool>>(name, array_bool_param_map_,
           values);
  }
...
private:
  ...
  template <typename T>
  bool ChangeValueFromMap(const std::string& name,
...
template <typename T>
bool ModelConfig::ChangeValueFromMap(const std::string& name,
                                     std::map<std::string, T>& container,
                                     const T& value) {
