...
public:
  ...
  int AddWork(const WorkItem& work_item) {
  ...
  void InformThreadStop() {
    is_work_alive_.store(false);
  }
...
public:
  ...
  int getIdleThreadHandle(int index = -1) {
    ...
    if (thread_content_vec_[index]->status_.load() != eThreadStatus::IDLE) return -1;
    return index;
  }
  ...
  int AddThreadWork(int thread_id, const WorkItem& work_item) {
    if (thread_id >= thread_content_vec_.size())
      return -1;
    return thread_content_vec_[thread_id]->AddWork(work_item);
  }
  ...
  int StopWork(int thread_id) {
    ...
    if (thread_content_vec_[thread_id]->status_.load() == eThreadStatus::WAITING) {
  ...
  int StopThread(int thread_id) {
    ...
    if (thread_content_vec_[thread_id]->status_.load() != eThreadStatus::STOPPED) {
      return -1;
    }
    return 0;
  }
