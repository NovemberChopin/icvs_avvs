...
#include "common/adapters/macro.h"
#include "ros/ros.h"

/**
 * @namespace avos::common::time
 * @brief avos::common::time
 */
namespace avos {
namespace common {
namespace time {
...
public:
  ...
  static Timestamp Now() {
    ...
    default:
      AERROR << "Unsupported clock mode: " << mode();
      exit(0);
    }
  }
  ...
  static void SetNow(const Duration &duration) {
    ...
    if (clock->mode_ != ClockMode::MOCK) {
      AERROR << "Cannot set now when clock mode is not MOCK!";
      exit(0);
    }
