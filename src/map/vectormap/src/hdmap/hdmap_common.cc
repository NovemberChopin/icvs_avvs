  ...
  for (const auto &curve : input_curve.segment()) {
    if (curve.has_line_segment()) {
      for (const auto &point : curve.line_segment().point()) {
        AVOS_CHECK(IsPointValid(point));
        points->emplace_back(point.x(), point.y());
      }
    } else {
      ROS_ERROR_STREAM( "Can not handle curve type." );
    }
  }
  RemoveDuplicates(points);
}

avos::common::math::Polygon2d ConvertToPolygon2d(const Polygon &polygon) {
  ...
  while (points.size() >= 2 && points[0].DistanceTo(points.back()) <=
         avos::common::math::kMathEpsilon) {
    points.pop_back();
  }
  return avos::common::math::Polygon2d(points);
}

void SegmentsFromCurve(
  const Curve &curve,
  std::vector<avos::common::math::LineSegment2d> *segments) {
  AVOS_RETURN_IF_NULL(segments);
...
int LaneInfo::PathDensify(std::vector<avos::common::math::Vec2d> &points_init, 
  ...
  avos::common::math::Vec2d each_point;
...
void LaneInfo::Init() {
  std::vector<avos::common::math::Vec2d> points_init;
  PointsFromCurve(lane_.central_curve(), &points_init);
  AVOS_CHECK_GE(points_init.size(), 2);
  segments_.clear();
  ...
  accumulated_s_.push_back(s);
  total_length_ = s;
  AVOS_CHECK(!unit_directions_.empty());
  ...
  if (lane_.has_type()) {
    if (lane_.type() == Lane::CITY_DRIVING) {
      ...
      ROS_ERROR_STREAM( "lane_[id = " << lane_.id().DebugString() << " type is NONE." );
    }
  } else {
    ROS_ERROR_STREAM( "lane_[id = " << lane_.id().DebugString() << "has NO type." );
  }
...
double LaneInfo::Curvature(const double s) const {
  if (points_.size() < 2) {
    ROS_ERROR_STREAM( "Not enough points to compute curvature." );
    return 0.0;
  }
  const double kEpsilon = 0.001;
  if (s + kEpsilon < accumulated_s_.front()) {
  ...
  if (iter == accumulated_s_.end()) {
    ROS_DEBUG_STREAM( "Reach the end of lane.");
    return 0.0;
  }
  int index = std::distance(accumulated_s_.begin(), iter);
  if (index == 0) {
    ROS_DEBUG_STREAM( "Reach the beginning of lane");
    return 0.0;
  } else {
...
bool LaneInfo::IsOnLane(const avos::common::math::Box2d &box) const {
...
PointENU LaneInfo::GetSmoothPoint(double s) const {
  ...
  if (delta_s < avos::common::math::kMathEpsilon) {
...
void LaneInfo::CreateKDTree() {
  avos::common::math::AABoxKDTreeParams params;
  ...
  for (size_t id = 0; id < segments_.size(); ++id) {
    ...
    segment_box_list_.emplace_back(
      avos::common::math::AABox2d(segment.start(), segment.end()), this,
      &segment, id);
  }
...
void JunctionInfo::Init() {
  ...
  AVOS_CHECK_GT(polygon_.num_points(), 2);
...
void StopSignInfo::UpdateOverlaps(const HDMapImpl &map_instance) {
  ...
  if (overlap_junction_ids_.size() <= 0) {
    ROS_WARN_STREAM( "stop sign " << id().id() << "has no overlap with any junction." );
  }
}
