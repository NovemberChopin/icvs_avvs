...
using avos::common::math::AABoxKDTreeParams;
...
int HDMapImpl::LoadMapFromFile(const std::string& map_filename) {
  ...
  if (avos::common::util::EndWith(map_filename, ".xml")) {
    if (!adapter::OpendriveAdapter::LoadData(map_filename, &map_)) {
      return -1;
    }
  } else if (!avos::common::util::GetProtoFromFile(map_filename, &map_)) {
    return -1;
  }
...
int HDMapImpl::GetSignals(const PointENU& point, double distance,
  ...
  return GetSignals({point.x(), point.y()}, distance, my_signals);
}

int HDMapImpl::GetSignals(const Vec2d& point, double distance,
                          std::vector<SignalInfoConstPtr>* my_signals) const {
  if (my_signals == nullptr || signal_segment_kdtree_ == nullptr) {
    return -1;
  }
  my_signals->clear();
  std::vector<std::string> ids;
  ...
  for (const auto& id : ids) {
    my_signals->emplace_back(GetSignalById(CreateHDMapId(id)));
  }
  return 0;
}
...
int HDMapImpl::GetNearestLane(const Vec2d& point,
                              LaneInfoConstPtr* nearest_lane, double* nearest_s,
                              double* nearest_l) const {
  //AVOS_RETURN_IF_NULL(nearest_lane);
  //AVOS_RETURN_IF_NULL(nearest_s);
  //AVOS_RETURN_IF_NULL(nearest_l);
...
int HDMapImpl::GetNearestLaneWithHeading(
  ...
  for (const auto& lane : lanes) {
    ...
    if (lane->lane().type() != avos::hdmap::Lane_LaneType::Lane_LaneType_CITY_DRIVING) {
...
int HDMapImpl::GetLanesWithHeading(const Vec2d& point, const double distance,
  ...
  for (auto& lane : all_lanes) {
    ...
    if (dis <= distance) {
      ...
      if (fabs(avos::common::math::NormalizeAngle(heading_diff)) <=
...
int HDMapImpl::GetRoadBoundaries(
  ...
  //AVOS_RETURN_IF_NULL(road_boundaries);
  //AVOS_RETURN_IF_NULL(junctions);
  ...
  for (const auto& lane : lanes) {
    ...
    if (road_ptr->has_junction_id()) {
      ...
      //AVOS_RETURN_IF_NULL(junction_boundary_ptr->junction_info);
...
int HDMapImpl::GetForwardNearestSignalsOnLane(
  ...
  std::vector<SignalInfoConstPtr>* my_signals) const {
  //AVOS_RETURN_IF_NULL(my_signals);
  ...
  int s_index = 0;
  avos::common::math::Vec2d car_point;
  car_point.set_x(point.x());
  car_point.set_y(point.y());
  avos::common::math::Vec2d map_point;
  if (GetLanes(point, kLanesSearchRange, &temp_surrounding_lanes) == -1) {
    ROS_INFO_STREAM( "Can not find lanes around car." );
    return -1;
  }
  ...
  if (surrounding_lanes.empty()) {
    ROS_INFO_STREAM( "Car is not on lane." );
    return -1;
  }
  for (const auto& lane : surrounding_lanes) {
    if (!lane->my_signals().empty()) {
      lane_ptr = lane;
      nearest_l =
  ...
  while (s < back_distance) {
    for (const auto& predecessor_lane_id : lane_ptr->lane().predecessor_id()) {
      ...
      if (lane_ptr->lane().turn() == avos::hdmap::Lane::NO_TURN) {
        break;
      }
    }
  ...
  while (lane_ptr != nullptr) {
    ...
    if (!min_dist_signal_ptr.empty() && unused_distance >= signal_min_dist) {
      *my_signals = min_dist_signal_ptr;
      break;
    }
    ...
    for (const auto& successor_lane_id : lane_ptr->lane().successor_id()) {
      ...
      if (tmp_lane_ptr->lane().turn() == avos::hdmap::Lane::NO_TURN) {
        break;
      }
    }
...
int HDMapImpl::GetStopSignAssociatedStopSigns(
  ...
  //AVOS_RETURN_IF_NULL(stop_signs);
  ...
  for (const auto& info_with_id : table) {
    ...
    for (size_t id = 0; id < info->segments().size(); ++id) {
      ...
      box_table->emplace_back(
        avos::common::math::AABox2d(segment.start(), segment.end()), info,
        &segment, id);
    }
  }
