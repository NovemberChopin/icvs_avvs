...
int RoadnetImpl::LoadRoadnetFromFile(const std::string& roadnet_filename) {
  Clear();
  if (avos::common::util::EndWith(roadnet_filename, ".xml")) {
    if (!adapter::OpendriveAdapter::LoadData(roadnet_filename, &roadnet_)) {
      return -1;
    }
  } else if (!avos::common::util::GetProtoFromFile(roadnet_filename, &roadnet_)) {
    return -1;
  }
...
int RoadnetImpl::LoadRoadnetFromProto(const Roadnet& roadnet_proto) {
  ...
  ROS_INFO_STREAM("load roadnodes in roadnet: "<<road_node_table_.size());
  return 0;
}
