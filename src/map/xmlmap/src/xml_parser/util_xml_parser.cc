...
int UtilXmlParser::ParseRegionBoundary(const tinyxml2::XMLElement& xml_node,
  ...
  AVOS_RETURN_IF_NULL(region_boundary);
  const tinyxml2::XMLElement* sub_node = xml_node.FirstChildElement("exterior_outline");
  AVOS_RETURN_IF_ERROR(!sub_node, "xml data missing exterior outline.");
  ...
  AVOS_RETURN_IF_ERROR(!boundary_lines_node, "xml data missing boundary line.");
  ...
  while (boundary_lines_node) {
    ...
    AVOS_RETURN_IF_ERROR(ParseGeometry(*geo_node, geometry),
  ...
  if (sub_node1) {
      ...
      while (sub_node1) {
        ...
        AVOS_RETURN_IF_ERROR(!boundary_lines_node1, "xml data missing boundary line.");
        ...
        while (boundary_lines_node1) {
          ...
          AVOS_RETURN_IF_ERROR(checker1 != tinyxml2::XML_SUCCESS,
                           "Error parsing boundary line id and order attributes");
          ...
          AVOS_RETURN_IF_ERROR(ParseGeometry(*geo_node1, geometry1),
                               "Error parsing ParseGeometry Geometry");
...
int UtilXmlParser::ParseOutline(const tinyxml2::XMLElement& xml_node,
  ...
  AVOS_RETURN_IF_ERROR(1, "Error outline object");
}
int UtilXmlParser::ParseCurbs(const tinyxml2::XMLElement& xml_node,
                              Curbs* curbs) {
  AVOS_RETURN_IF_NULL(curbs);
...
int UtilXmlParser::ParsePosition(const tinyxml2::XMLElement& xml_node,
  ...
  AVOS_RETURN_IF_ERROR(ParsePoint(xml_node, point),
...
int UtilXmlParser::ParseBoundary(const tinyxml2::XMLElement& xml_node,
        avos::xmlmap::Boundary* boundary)
{
  AVOS_RETURN_IF_NULL(boundary);
  const auto sub_node = xml_node.FirstChildElement("boundary");
  AVOS_CHECK(sub_node != nullptr);
  int checker = tinyxml2::XML_SUCCESS;
  double xg = 0.0;
  double yg = 0.0;
  double length = 0.0;
