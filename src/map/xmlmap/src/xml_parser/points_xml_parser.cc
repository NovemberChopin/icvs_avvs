...
int PointsXmlParser::Parse(const tinyxml2::XMLElement& xml_node,
  ...
  // AVOS_RETURN_IF_ERROR(!points_node, "xml data missing points.");
  ...
  while (sub_node) {
    ...
    AVOS_RETURN_IF_ERROR(!sub_node2, "xml data missing position.");
    Position position;
    AVOS_RETURN_IF_ERROR(UtilXmlParser::ParsePosition(*sub_node2, &position),
