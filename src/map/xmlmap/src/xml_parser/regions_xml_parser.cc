...
int RegionsXmlParser::Parse(const tinyxml2::XMLElement& xml_node,
  ...
  if(!regions_node)
      return 0;
  //AVOS_RETURN_IF_ERROR(!regions_node, "xml data missing regions.");
  ...
  while (sub_node) {
    ...
    AVOS_RETURN_IF_ERROR(!sub_node2, "xml data missing outline.");
    Outlines outlines;
    while(sub_node2) {
       Outline outline;
       AVOS_RETURN_IF_ERROR(UtilXmlParser::ParseOutline(*sub_node2, &outline),
    ...
    Curbs curbs;
    if(sub_node3) {
        AVOS_RETURN_IF_ERROR(!sub_node3, "xml data missing curbs.");
        AVOS_RETURN_IF_ERROR(UtilXmlParser::ParseCurbs(*sub_node3, &curbs),
