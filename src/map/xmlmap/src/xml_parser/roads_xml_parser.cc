...
int RoadsXmlParser::Parse(const tinyxml2::XMLElement& xml_node, Roads* roads) {
  ...
  if(!road_node)
      return 0;
  //AVOS_RETURN_IF_ERROR(!road_node, "xml data missing roads.");
  ...
  while (sub_node) {
    ...
    if (sub_node1) {
       AVOS_RETURN_IF_ERROR(!sub_node1, "xml data missing contact boundary.");
    ...
    AVOS_RETURN_IF_ERROR(!sub_node2, "xml data missing geometry.");
    Geometry geo;
    AVOS_RETURN_IF_ERROR(UtilXmlParser::ParseGeometry(*sub_node2, &geo),
