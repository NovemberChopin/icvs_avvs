...
int LocationXmlParser::Parse(const tinyxml2::XMLElement& xml_node, PathPoint* pathpoint) {
  ...
  // AVOS_RETURN_IF_ERROR(!location_node, "xml data missing pathPoint.");
  ...
  while (sub_node) {
    ...
    // AVOS_RETURN_IF_ERROR(!sub_node2, "xml data missing centerPoint.");

    Point center_point;
    AVOS_RETURN_IF_ERROR(UtilXmlParser::ParsePoint(*sub_node, &center_point),
