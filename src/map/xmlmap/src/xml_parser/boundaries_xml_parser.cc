...
int BoundariesXmlParser::Parse(const tinyxml2::XMLElement& xml_node,
  ...
  if (boundaries_node) {
    AVOS_RETURN_IF_ERROR(!boundaries_node, "xml data missing boundaries.");
    const tinyxml2::XMLElement* sub_node = boundaries_node->FirstChildElement("boundary");
    if (sub_node) {
      AVOS_RETURN_IF_ERROR(!sub_node, "xml data missing boundary.");
      while (sub_node) {
        RegionBoundary region_boundary;
        AVOS_RETURN_IF_ERROR(UtilXmlParser::ParseRegionBoundary(*sub_node, &region_boundary),
