...
int main(int argc, char **argv) {
	...
	avos::common::AvosNode::Init("generator", Enable_GLOG_Screen);
  ...
  if ((ExistFile(file + "_hdmap.bin") != 0) ||
    ...
    AVOS_CHECK(avos::hdmap::adapter::OpendriveAdapter::LoadData(
  ...
  if (ExistFile(output_file) == 0) {
    ...
    AVOS_CHECK(avos::common::util::SetProtoToBinaryFile(pb_map, output_file));
  ...
  if (ExistFile(output_file) == 0) {
    ...
    AVOS_CHECK(
      avos::common::util::SetProtoToBinaryFile(pb_roadnet, output_file));
    AINFO << "creat: " << output_file;
  }
  ...
  if (ExistFile(output_file) == 0) {
    ...
    AVOS_CHECK(
      avos::common::util::SetProtoToASCIIFile(pb_map, output_file));
    AINFO << "creat: " << output_file;
  }
  ...
  if (ExistFile(output_file) == 0) {
    ...
    AVOS_CHECK(
      avos::common::util::SetProtoToASCIIFile(pb_roadnet, output_file));
    AINFO << "creat: " << output_file;
  }
