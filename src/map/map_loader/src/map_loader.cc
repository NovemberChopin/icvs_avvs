...
std::shared_ptr<VectorMap> MapLoader::CreateVectorMap() {
  ...
  if (vectormap->LoadMapFromFile(xml_file) != 0) {
    ROS_ERROR_STREAM("Failed to load VectorMap " << xml_file);
    return nullptr;
  }
  ...
  if (vectormap->LoadRoadnetFromFile(xml_file) != 0) {
    ROS_ERROR_STREAM("Failed to load roadnet " << xml_file);
    return nullptr;
  }
  ROS_INFO_STREAM("Load VectorMap and roadnet success: ");
  return vectormap;
}
...
std::shared_ptr<LinkMap> MapLoader::CreateLinkMap() {
  ...
  if (!linkmap->LoadMapFromFile(xml_file, mapinfo->GetMapPath() + mpheader.wxb_map_dir)) {
    ROS_ERROR_STREAM("Failed to load linkMap " << xml_file);
    return nullptr;
  }
...
avos::map::LinkMap* MapLoader::GetLinkmapPtr() {
  Mapinfo* mapinfo = GetMapinfoPtr();
  if (mapinfo == nullptr) {
    return nullptr;
  }
...
avos::map::AppMap *MapLoader::GetAppmapPtr() {
  Mapinfo* mapinfo = GetMapinfoPtr();
  if (mapinfo == nullptr) {
    return nullptr;
  }
...
bool MapLoader::SetCurrentMap(std::string map_id)
{
  if (!mapinfo_->SetCurrentMap(map_id)) {
    ROS_ERROR_STREAM("mapinfo_->SetCurrentMap failed");
    return false;
  }
...
bool MapLoader::UpdateMapConfig()
{
