...
int LoadVectorFile(PointGCCS &car_gccs) {
	...
	auto mapinfo = avos::map::MapLoader::GetMapinfoPtr();
  ...
  if (mpheader.WxbmapEnabled) {
    avos::map::LinkMap* link_map = avos::map::MapLoader::GetLinkmapPtr();
...
void GetPathEndPoint(PointGCCS &output_pgccs)
{
    for (auto lane_info_map : glane_info_map_vec) 
    {
        for (auto it = lane_info_map->begin(); it != lane_info_map->end(); ++it) 
        {
            if (it->second.gccs_point.size() > 0) 
            {
                output_pgccs.xg = it->second.gccs_point[it->second.gccs_point.size()-1].xg;
                output_pgccs.yg = it->second.gccs_point[it->second.gccs_point.size()-1].yg;
                output_pgccs.angle = it->second.gccs_point[it->second.gccs_point.size()-1].angle;
            }
        }
    }
}

void GetPathStartPoint(PointGCCS &output_pgccs)
{
    for (auto lane_info_map : glane_info_map_vec) 
    {
        for (auto it = lane_info_map->begin(); it != lane_info_map->end(); ++it) 
        {
            if (it->second.gccs_point.size() > 0) 
            {
                output_pgccs.xg = it->second.gccs_point[0].xg;
                output_pgccs.yg = it->second.gccs_point[0].yg;
                output_pgccs.angle = it->second.gccs_point[0].angle;
            }
        }
    }  
}