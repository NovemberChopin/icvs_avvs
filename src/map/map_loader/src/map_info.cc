...
bool Mapinfo::SetCurrentMap(std::string map_id)
{
  tinyxml2::XMLDocument doc;
  std::string path = GetMapsBasePasth() + "map.xml";
  ...
  node->SetAttribute("default_name", map_id.c_str());
...
int Mapinfo::LoadMapInfo() {
  ...
  AINFO << std::fixed <<"2 miheader_.center_point: gccs (" << miheader_.center_point.xg << ", " << miheader_.center_point.yg << ")";
...
int Mapinfo::GetParams() {
  avos::common::config::MultiModelConfigProto configs;
  std::string map_param = GetMapPath() + "map_param.conf";
  std::cout << "map config file: " << map_param << std::endl;
  if (!avos::common::util::GetProtoFromFile(map_param, &configs)) {
