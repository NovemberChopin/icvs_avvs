...
Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh) : data_(new lslidar_rawdata::RawData())
{
  ...
  private_nh.param("output_points_topic", output_points_topic, std::string("lidar_points_2"));
  ...
  private_nh.param("input_packets_topic", input_packets_topic, std::string("lidar_packets_2"));
  ...
  dynamic_reconfigure::Server<lslidar_c32::lslidar_c32DynamicConfig>::CallbackType f;
  f = boost::bind(&Convert::SetDynamicConfigCb, this, _1, _2);
  dynamic_server_.setCallback(f);

}

void Convert::SetDynamicConfigCb(lslidar_c32::lslidar_c32DynamicConfig &config, uint32_t level) {
  data_->SetRollAndPitch(config.roll, config.pitch);
  data_->loadConfigFile(node_, private_nh_);  // load lidar parameters
}
