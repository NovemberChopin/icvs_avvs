...
void RawData::loadConfigFile(ros::NodeHandle node, ros::NodeHandle private_nh)
{
  ...
  private_nh.param("car_left", car_left_, 0.0);
  private_nh.param("car_right", car_right_, 0.0);
  private_nh.param("car_front", car_front_, 0.0);
  private_nh.param("car_back", car_back_, 0.0);
  private_nh.param("car_top", car_top_, 0.0);
  private_nh.param("car_bottom", car_bottom_, 0.0);
  private_nh.param("Xoffset", Xoffset_, 0.0);
  private_nh.param("Yoffset", Yoffset_, 0.0);
  private_nh.param("Zoffset", Zoffset_, 0.0);
  // private_nh.param("Roll", Roll_, 0.0);
  // private_nh.param("Pitch", Pitch_, 0.0);
  ...
  for (uint16_t rotIndex = 0; rotIndex < ROTATION_MAX_UNITS; ++rotIndex) {
    ...
    cos_rot_table_[rotIndex] = cosf(rotation);
    sin_rot_table_[rotIndex] = sinf(rotation);
  }
  int xr = Roll_ >= 0 ?
           Roll_ * 100 : (Roll_ + 360) * 100;
  int yr = Pitch_ >= 0 ?
           Pitch_ * 100 : (Pitch_ + 360) * 100;
  int zr = Yaw_ >= 0 ?
           Yaw_ * 100 : (Yaw_ + 360) * 100;
  //  transform x
  transform_x_of_x = cos_rot_table_[zr] * cos_rot_table_[yr];
  transform_y_of_x = sin_rot_table_[zr] * cos_rot_table_[xr] +
                     cos_rot_table_[zr] * sin_rot_table_[yr] * sin_rot_table_[xr];
  transform_z_of_x = sin_rot_table_[zr] * sin_rot_table_[xr] -
                     cos_rot_table_[zr] * sin_rot_table_[yr] * cos_rot_table_[xr];
  //  transform y
  transform_x_of_y = -sin_rot_table_[zr] * cos_rot_table_[yr];
  transform_y_of_y = cos_rot_table_[zr] * cos_rot_table_[xr] -
                     sin_rot_table_[zr] * sin_rot_table_[yr] * sin_rot_table_[xr];
  transform_z_of_y = cos_rot_table_[zr] * sin_rot_table_[xr] +
                     sin_rot_table_[zr] * sin_rot_table_[yr] * cos_rot_table_[xr];
  //  transform z
  ...
  difop_sub_ = node.subscribe("lidar_packets_2_difop", 10, &RawData::processDifop, (RawData*)this);
}

void RawData::SetRollAndPitch(double roll, double pitch) {
    Roll_ = roll;
    Pitch_ = pitch;
}
  ...
  for (int block = 0; block < BLOCKS_PER_PACKET; block++, this->block_num++)  // 1 packet:12 data blocks
  {
    ...
    //pcl::PointXYZI point;
        ...
        else
        {
            ...
            /// ========add by sunzhenye for points extrinsic calibration 20191119=========
            VPoint point_tmp;
            point_tmp.x = -point.y;
            point_tmp.y = point.x;
            point_tmp.z = point.z;

            point.x = transform_x_of_x * point_tmp.x + transform_y_of_x * point_tmp.y + transform_z_of_x * point_tmp.z;
            point.y = transform_x_of_y * point_tmp.x + transform_y_of_y * point_tmp.y + transform_z_of_y * point_tmp.z;
            point.z = transform_x_of_z * point_tmp.x + transform_y_of_z * point_tmp.y + transform_z_of_z * point_tmp.z;
            point.x += Xoffset_;
            point.y += Yoffset_;
            point.z += Zoffset_;
            // point.ring = dsr;
            point.angle = azimuth_corrected_f * 0.01;
            point.angle = (point.angle + 180.0) > 360.0 ? point.angle - 180.0 : point.angle + 180.0;

            if (point.y < car_left_ && point.y > car_right_ &&
                point.x < car_front_ && point.x > car_back_ &&
                point.z < car_top_ && point.z > car_bottom_) {
              point.x = NAN;
              point.y = NAN;
              point.z = NAN;
              point.intensity = 0;
